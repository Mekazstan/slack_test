...
app_bolt = App(token=SLACK_BOT_TOKEN, signing_secret=SLACK_APP_TOKEN)
...
@app_bolt.action("confirm_forward_button")
def handle_confirm_forward(ack, body, client):
    ack()
...
if __name__ == "__main__":
    app_flask.run(debug=True, host="0.0.0.0", port=int(os.environ.get("PORT", 3000)))